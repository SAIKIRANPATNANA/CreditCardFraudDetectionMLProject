...
class DataTransformation:
    ...
    def get_data_transformation_object(self):
        try:
            ...
            feature_pipeline = Pipeline(steps=[('std_scaler',StandardScaler())])
            feature_cols = ['LIMIT_BAL', 'SEX', 'EDUCATION', 'MARRIAGE', 'AGE', 'PAY_0', 'PAY_2','PAY_3', 'PAY_4', 'PAY_5', 'PAY_6', 'BILL_AMT1', 'BILL_AMT2','BILL_AMT3', 'BILL_AMT4', 'BILL_AMT5', 'BILL_AMT6', 'PAY_AMT1','PAY_AMT2', 'PAY_AMT3', 'PAY_AMT4', 'PAY_AMT5', 'PAY_AMT6','default payment next month']
            preprocessor = ColumnTransformer([('feature_pipeline',feature_pipeline,feature_cols)])
            logging.info(('Transformation Pipeline is created'))
            return preprocessor
        except Exception as e:
            logging.info("Exception 0ccured in getting Object of Data Transformation")
            return(e,sys)
    def initiate_data_transformation(self,train_data_path,test_data_path):
        try:
            ...
            drop_col = [target_col]
            input_features_train_df = train_df.drop(columns=drop_col,axis=1)
            target_feature_train_df = train_df[target_col]
            input_features_test_df = test_df.drop(columns=drop_col,axis=1)
            ...
            # save_obj(file_path=self.data_transformation_config.preprocesssor_obj_file_path,file_obj=preprocessing_obj)
            logging.inof('Data Transformation is completed')
            return(train_array,test_array,self.data_transformation_config.preprocesssor_obj_file_path)
        except Exception as e:
            logging.info('Exception occured in the Data Transformation Initiation')
            return (e,sys)
