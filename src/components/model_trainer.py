...
# from sklearn.utils import evaluate_model
...
class ModelTrainer:
    ...
    def initiate_model_training(self,train_array,test_array):
        try:
            logging.info('Splitting the dependent and independent data from train and test datasets')
            x_train,x_test,y_train,y_test = train_array[:,:-1],test_array[:,:-1],train_array[:,-1],test_array[:,-1]
            models = {'random_forest_clf':RandomForestClassifier(),'xg_boost_clf':XGBClassifier()}
            # model_report:dict = evaluate_model(x_train,y_train,x_test,y_test,models)
            # print(model_report)
            # logging.info(f'Model Report : {model_report}')
            # best_model_score = 0
            # for i in model_report.values():
            #     if best_model_score<i:
            #         best_model_score = i
            ...
            logging.info(('Model Training is compled'))
        except Exception as e:
            logging.info('Exception occured in  the initiation of Model Training')
            return(e,sys)
